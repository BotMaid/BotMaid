class pluginClass:
    ...
    def __init__(self):
        ...
        for key, reply, group in cursor.execute('select * from keywords').fetchall():
            if not self.keywordsDict.get(group, {}):
                self.keywordsDict[group] = {}
            self.keywordsDict[group][key] = reply
        cursor.close()
        conn.close()
    ...
    async def onMessageReceivedListener(self, client, event, rawMsg):
        if not self.keywordsDict:
            return
        try:
            for key in self.keywordsDict[str(event.chat_id)].keys():
                if key in rawMsg:
                    await client.send_message(event.chat_id, self.keywordsDict[str(event.chat_id)][key],
